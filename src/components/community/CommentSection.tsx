    ...
    <div className="flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        {loading && comments.length === 0 && (
          <div className="p-4 text-center text-sm" style={{ color: "var(--text-secondary)" }}>{t("community.comments.loading")}</div>
        )}
        {!loading && comments.length === 0 && (
          <div className="p-4 text-center text-sm" style={{ color: "var(--text-secondary)" }}>{t("community.comments.empty")}</div>
        )}
        {comments.map((c) => (
          <div key={c.id} className="px-4 py-3 flex gap-3" style={{ borderBottom: "1px solid var(--border-color)" }}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-xs" style={{ color: "var(--text-color)" }}>{c.author?.displayName ?? t("community.user.unknown")}</span>
                <span className="text-[10px]" style={{ color: "var(--text-secondary)" }}>{timeAgo(c.createdAt)}</span>
              </div>
              {c.deletedAt ? (
                <p className="text-xs italic mt-0.5" style={{ color: "var(--text-secondary)" }}>{t("community.comments.deleted")}</p>
              ) : (
