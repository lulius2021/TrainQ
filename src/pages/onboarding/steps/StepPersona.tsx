    ...
    return (
        ...
        >
            {PERSONAS.map((p) => {
              ...
              ${isSelected
                                ? 'bg-[var(--card-bg)] border-[var(--accent-color)] shadow-[0_8px_30px_rgba(59,130,246,0.15)] ring-1 ring-[var(--accent-color)]'
                                : 'bg-[var(--card-bg)] border-white/5 opacity-80 hover:opacity-100 hover:border-white/10'}
            `}
                    >
                        <span className="text-4xl mb-1 block transform transition-transform group-hover:scale-110">{p.icon}</span>
                        <div>
                            <div className={`text-xl font-bold mb-1 ${isSelected ? 'text-[var(--accent-color)]' : 'text-[var(--text)]'}`}>{p.label}</div>
                            <div className="text-base text-[var(--text-muted)] leading-snug">{p.desc}</div>
                        </div>
                    </button>
                );
            })}
